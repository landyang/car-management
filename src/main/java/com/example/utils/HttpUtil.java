...
/**
 *发送http请求工具类（HttpClient）
 *
 * @author HaN
 * @create 2019-05-09 16:36
 */
public class HttpUtil {
    private static Logger log = LoggerFactory.getLogger(HttpUtil.class);

    private static final String USER_AGENT = "user-agent";
    private static final String USER_AGENT_VALUE = "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/29.0.1547.66 Safari/537.36";
...
//    private static final String SSL = "ssl";
    ...
    public static void main(String[] args) {
        ...
        String param = "province=HN&dtype&format&callback&key=870b94a3291d8553a3bfddd49279717e";
        ObjectMapper mapper = new ObjectMapper();
        String jsonStr = null;
        try {

            jsonStr = sendGet(url, param);
            System.out.println(jsonStr);
            //jackson解析字符串
            //jsonStr是一个json字符串
            JsonNode rootNode = mapper.readTree(jsonStr);
            JsonNode targetNode = null;
            targetNode = rootNode.findValue("reason");
            System.out.println(targetNode);
            //JsonUtil解析字符串（对jackson的方法封装）
            System.out.println(JsonUtil.findValue(jsonStr, "reason"));
